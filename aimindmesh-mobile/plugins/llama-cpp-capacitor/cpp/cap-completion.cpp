...
void llama_cap_context_completion::rewind() {
    ...
    parent_ctx->params.sampling.grammar = common_grammar();
    num_prompt_tokens = 0;
...
void llama_cap_context_completion::loadPrompt(const std::vector<std::string> &media_paths) {
    ...
    if (!has_media) {
        ...
        // Check if prompt starts with <|im_start|> (ChatML) or other indicators that BOS might not be needed
        bool add_special = true;
        if (parent_ctx->params.prompt.find("<|im_start|>") == 0) {
            add_special = false;
             LOG_INFO("Disabling BOS token (add_special=false) because prompt starts with <|im_start|>");
        }

        // Text-only path
        // Enable special token parsing (4th arg = true)
        text_tokens = ::common_tokenize(parent_ctx->ctx, parent_ctx->params.prompt, add_special, true);
        ...
        for (auto& token : text_tokens) {
            std::vector<llama_token> single_token_vec = {token};
            ss << token << " '" << tokens_to_str(parent_ctx->ctx, single_token_vec.cbegin(), single_token_vec.cend()) << "' ";
        }
        LOG_INFO("%s\n", ss.str().c_str());
