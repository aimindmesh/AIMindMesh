declare module 'llama-cpp-capacitor' {
  ...
  // =========================================================================
  // SmolLM Streaming API (New Architecture based on SmolChat-Android)
  // =========================================================================

  /**
   * Parameters for initializing a SmolLM streaming context.
   */
  export interface SmolLMParams {
    /** Path to the GGUF model file */
    model: string;
    /** Sampling temperature (default: 0.7) */
    temperature?: number;
    /** Minimum probability for top-p sampling (default: 0.1) */
    min_p?: number;
    ...
    use_mlock?: boolean;
    /** Use Vulkan GPU acceleration (default: false) */
    use_vulkan?: boolean;
    /** Store conversation history (default: true) */
    store_chats?: boolean;
    /** Custom Jinja chat template */
    chat_template?: string;
    /** System prompt to add on initialization */
    system_prompt?: string;
  }

  /**
   * Result from SmolLM initialization.
   */
  export interface SmolLMInitResult {
    contextId: number;
    success: boolean;
    architecture: 'smollm';
  }
  ...
  /**
   * Result from SmolLM completion.
   */
  export interface SmolLMCompletionResult {
   ...
   */
  export interface SmolLMTokenEvent {
    contextId: number;
    token: string;
  }

  /**
   * Metrics from SmolLM generation.
   */
  export interface SmolLMMetrics {
    tokensPerSecond: number;
    contextUsed: number;
  ...
  /**
   * SmolLM Streaming API Plugin Interface.
   * Based on SmolChat-Android for improved performance and stability.
   */
  export interface SmolLMPlugin {
    /**
     * Initialize a SmolLM context for streaming inference.
     */
    initSmolLM(options: {
      contextId: number;
      model: string;
      params?: SmolLMParams;
    }): Promise<SmolLMInitResult>;

    /**
     * Generate a streaming response. Emits 'smolLMToken' events during generation.
     */
    streamingCompletion(options: {
      contextId: number;
      prompt: string;
    }): Promise<SmolLMCompletionResult>;

    /**
     * Generate a complete response (non-streaming).
     */
    smolLMCompletion(options: {
      contextId: number;
      prompt: string;
    }): Promise<SmolLMCompletionResult>;
     ...
     */
    interruptSmolLM(options: { contextId: number }): Promise<void>;

    /**
     * Get generation metrics.
     */
    getSmolLMMetrics(options: { contextId: number }): Promise<SmolLMMetrics>;

    /**
     * Release a SmolLM context.
     */
    releaseSmolLM(options: { contextId: number }): Promise<void>;

    /**
     * Release all SmolLM contexts.
     */
    releaseAllSmolLM(): Promise<void>;

    /**
     * Add a listener for token events during streaming.
     */
    addListener(
      eventName: 'smolLMToken',
      listener: (event: SmolLMTokenEvent) => void
    ): { remove: () => void };
  }
}
